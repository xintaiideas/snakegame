...
public class Snake {
	...
	//给蛇添加成员，蛇身就是由几个方块组成的
	private boolean life =true;//默认值为true否则不循环
   ...
   private Point tail;//存储尾巴的，定义成成员变量不然其他访问不到
	...
	public void move() {
		...
		int y = body.getFirst().y;//假如body.getLast();将会出现蛇身体乱了套
		/*要获得新的头部
		 * 要确定方向，才能知道新的头部
		 * 在初始化构造蛇身的时候默认方向是往右的
		 * 我们定义出所有方向
